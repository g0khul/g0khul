...
import { tapAnimation, appleEase, hoverSlide } from "../config/animations";
...
const RSS_API = `https://api.rss2json.com/v1/api.json?rss_url=${encodeURIComponent(SUBSTACK_URL + "/feed")}`;
  ...
  return (
    <div className="flex-1 flex flex-col gap-12 py-8">
      <div className="grid md:grid-cols-[auto_1fr] gap-8 items-start">
        ...
        >
          <img src={bio.photo} alt="Profile" className="w-full h-full object-cover" />
        </motion.div>
          ...
          <div className="flex flex-col gap-4">
            {bio.paragraphs.map((paragraph, index) => (
              <motion.p
                ...
                className={`text-base leading-relaxed m-0 ${paragraph.startsWith('"') ? "italic opacity-90" : ""}`}
              >
                {paragraph}
              </motion.p>
            ))}
      ...
      <div className="grid md:grid-cols-2 gap-8 md:gap-12">
        <div>
          <h2 className="text-lg font-bold mb-6 uppercase tracking-wide">Journey</h2>
          <div className="relative pl-6 border-l-2 border-current">
            {timeline.map((entry, index) => (
              <motion.div
                ...
                transition={{ duration: 0.4, delay: index * 0.1, ease: appleEase }}
              ...
              >
                ...
                <div className="flex flex-col gap-1">
                  ...
                  <p className="text-sm opacity-70 m-0 mt-1">{entry.description}</p>
                </div>
              </motion.div>
            ))}
          </div>
        ...
        <div className="flex flex-col gap-8">
          <div>
            <h2 className="text-lg font-bold mb-6 uppercase tracking-wide">Latest Writings</h2>
            {loading ? (
              ...
              <div className="flex flex-col gap-4">
                {posts.map((post, index) => (
                  ...
                  >
                    <span className="text-sm opacity-60 block">{formatDate(post.pubDate)}</span>
          ...
          <div>
            <h2 className="text-lg font-bold mb-6 uppercase tracking-wide">Killed by Gokhul</h2>
            <p className="text-sm opacity-60 mb-4">Things I started but never finished. RIP.</p>
            <div className="flex flex-col gap-4">
                ...
                >
                  {item.link ? (
                    ...
                    >
                      ...
                      <span className="text-sm opacity-50 ml-2">({item.causeOfDeath})</span>
                    </motion.a>
                  ) : (
                    <div>
                  ...
                  <p className="text-sm opacity-60 m-0 mt-1">{item.description}</p>
                </motion.div>
              ))}
            </div>
          </div>
